...
async function fetchText(url) {
  const res = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; StAndrewsRoomWatch/1.0)' } });
...
function titleCase(str) {
  ...
  return str.toLowerCase().replace(/\b\w/g, c => c.toUpperCase()).replace(/\bKy(\d)/gi, 'KY$1');
}
...
module.exports = async (req, res) => {
  ...
  const sources = [
    { name: 'St Andrews Property Lets', url: 'https://standrewspropertylets.uk/',
      parse: (text) => parseSAPL(text).filter(i => !/property let/i.test(i.status)).map(i => ({ source: 'St Andrews Property Lets', tag: 'src-sapl', address: i.address, price: '', url: 'https://standrewspropertylets.uk/' })) },
    { name: 'Bradburne & Co', url: 'https://www.bradburne.co.uk/lettings/fife/st-andrews/',
      parse: (text) => parseBradburne(text).filter(i => !/let agreed/i.test(i.status)).map(i => ({ source: 'Bradburne & Co', tag: 'src-brad', address: i.address, price: i.rent || '', url: i.url })) },
    { name: '55Rent', url: 'https://55rent.co.uk/properties.html',
      parse: (text) => parse55Rent(text).filter(i => !/tenancy agreed/i.test(i.statusOrPrice)).map(i => ({ source: '55Rent', tag: 'src-55r', address: i.address, price: i.statusOrPrice, url: i.url })) }
  ];
  ...
  res.status(200).json({ listings, errors, checkedAt: new Date().toISOString() });
};
